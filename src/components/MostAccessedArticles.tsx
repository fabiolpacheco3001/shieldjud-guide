...
import { getTopArticleSlugs } from "@/lib/articleViews";
import { getArticleBySlug } from "@/lib/articleLoader";

const MostAccessedArticles = () => {
  const navigate = useNavigate();
  const topSlugs = getTopArticleSlugs(5);

  const articles = topSlugs
      ...
      <div className="container max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center">
            <TrendingUp className="text-secondary-foreground" size={20} />
          </div>
...
export default MostAccessedArticles;
